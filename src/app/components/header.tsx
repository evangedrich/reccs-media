    ...
    return (
        <div className="w-full sticky top-0 bg-[var(--color-back)] z-30">
            <div className="w-full border-b-2 border-solid border-[var(--color-front)] flex items-center justify-between overflow-hidden">
                ...
                <div className={`w-13 h-13 text-center leading-13 flex items-center ${showGlobe&&(pathname==="/"||pathname.slice(0,8)==="/regions")?"justify-end max-sm:justify-center":"justify-center max-sm:justify-center"} active:scale-90 transition-[scale]`}>
                    <div className={`h-6 ${showGlobe?"rounded-[1px] w-10 max-sm:rounded-full max-sm:w-6":"rounded-full w-6 max-sm:rounded-[1px] max-sm:w-6"} overflow-hidden ring-[1.8px] ring-[var(--color-front)] cursor-pointer group hover:opacity-80 ${pathname==="/"||pathname.slice(0,8)==="/regions"?"block":"hidden"}`} onClick={() => toggleGlobe()}>
                    ...
                    <div className={`w-7 h-7 pr-2 ${pathname==="/"||pathname.slice(0,8)==="/regions"?"hidden":"flex"} items-center justify-start cursor-pointer`} onClick={() => router.back()}>
