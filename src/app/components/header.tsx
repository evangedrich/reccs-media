            ...
            <div className={`w-full border-b-2 border-solid border-[var(--color-front)] ${showWidget ? "max-h-10" : "max-h-0 mt-[-2px]"} transition-[max-height]`}>
                <div className={`flex justify-between px-1 h-8 transition-opacity ${showWidget ? "" : "opacity-0 pointer-events-none flex-wrap"}`}>
                    <div className="m-1 sm:pl-1 pl-2 items-center sm:gap-8 gap-5 flex">
                        {isDark
                        ? <div className="sm:after:content-['dark'] sm:hover:after:content-['light'] sm:hover:italic cursor-pointer" onClick={() => setIsDark(!isDark)}><span className="align-[0.2em] inline-block transform sm:scale-100 scale-150">{colonVisible ? "◒" : "◓"}</span> </div>
                        : <div className="sm:after:content-['light'] sm:hover:after:content-['dark'] sm:hover:italic cursor-pointer" onClick={() => setIsDark(!isDark)}><span className="align-[0.2em] inline-block transform sm:scale-100 scale-150">{colonVisible ? "◐" : "◑"}</span> </div>
                        }
                        {isOffline
                        ? <div className="sm:after:content-['offline'] sm:hover:after:content-['online'] sm:hover:italic cursor-pointer" onClick={() => setIsOffline(!isOffline)}><span className="align-[0.2em] inline-block transform sm:scale-100 scale-150">{colonVisible ? "○" : "∅"}</span> </div>
                        : <div className="sm:after:content-['online'] sm:hover:after:content-['offline'] sm:hover:italic cursor-pointer" onClick={() => setIsOffline(!isOffline)}><span className="align-[0.2em] inline-block transform sm:scale-100 scale-150">{colonVisible ? "⦾" : "⦿"}</span> </div>
                        }
                    </div>
